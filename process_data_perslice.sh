...
segment_and_label_if_does_not_exist(){
  ...
  if [ -e "${FILELABELMANUAL}.nii.gz" ]; then
    ...
    # Remove label disc 1 to avoid errors if seg doesn't cover C1 label
    sct_label_utils -i "${FILELABEL}.nii.gz" -remove 1 -o "${FILELABEL}.nii.gz"
...
mkdir -p $PATH_RESULTS/results_perslice/
# Copy source images
...
FILE_SEG_LABEL=${file}_seg_labeled
...
for r_coef in ${R_COEFS[@]}; do
  ...
  for i_transfo in ${seq_transfo[@]}; do
    ...
    #sct_deepseg -i ${file_r_t}.nii.gz -task seg_sc_contrast_agnostic -thr 0 # -qc $PATH_QC -qc-subject ${SUBJECT}
    # TODO: soft csa?
    ...
    # Compute slicewise CSA between C2 and C5 levels
    start=`date +%s`
    sct_process_segmentation -i $file_r_t_seg.nii.gz -perslice 1 -vertfile $file_label_r_t.nii.gz -o $PATH_RESULTS/results_perslice/csa_perslice_${SUBJECT}_t${i_transfo}_${r_coef}.csv
    end=`date +%s`
    runtime=$((end-start))
