...
segment_and_label_if_does_not_exist(){
  ...
  FILESEG="${file}_seg"
  sct_deepseg -i ${file}.nii.gz -task seg_sc_contrast_agnostic -largest 1 -o ${FILESEG}.nii.gz #-thr 0 #-qc $PATH_QC -qc-subject ${SUBJECT}
...
for r_coef in ${R_COEFS[@]}; do
  ...
  for i_transfo in ${seq_transfo[@]}; do
    ...
    sct_deepseg -i ${file_r_t}.nii.gz -task seg_sc_contrast_agnostic -largest 1 -o ${file_r_t}_seg.nii.gz #-thr 0 # -qc $PATH_QC -qc-subject ${SUBJECT}
    # TODO: soft csa?
